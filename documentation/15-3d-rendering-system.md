...
Scene graph nodes:
    ...
    MeshGroup3D         ← Group container for organizing meshes
...
- `drawSelectionBox(pass, meshes, camera, hoveredCorner)` — draws the OBB wireframe + corner spheres; `hoveredCorner` (0–7) highlights the hovered corner in yellow
- `hitTest(rayOrigin, rayDir, meshes, camera, mode)` — returns hovered `GizmoAxis` (`'x'|'y'|'z'|'xy'|'xz'|'yz'|null`)
- `hitTestCorner(rayOrigin, rayDir, meshes, camera)` — ray-sphere test against the 8 OBB corners; returns corner index (0–7) or `null`
...
**Frustum culling** still uses a world AABB (from `getMeshWorldAABB3D` in `Renderer3D`), which is derived from the OBB corners. Over-rejection of off-screen meshes is acceptable; under-rejection of on-screen meshes is not. The selection UI uses OBB exclusively.

### Corner Scale Handles

When scale mode is active, all 8 OBB corner spheres become interactive. Dragging a corner scales the mesh while holding the **opposite corner** (the anchor) fixed in world space — matching the Spline 3D editor's corner-drag behavior.
...
pickMesh(mouseX, mouseY, w, h, camera, meshes)
  ...
  → Returns { mesh, hitPoint, distance } or null
```
...
Portable `.frogmarks` ZIP archives bundle all project data for local download — raster layers, vector scene, 3D nodes with imported GLB files, textures, and animation. See `notes/frogmarks-project-package-spec.md` for full integration details.
