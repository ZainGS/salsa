...
9. **Static thumbnail fallback** — Include a canvas thumbnail in the `.frogmarks` manifest (Frogmarks captures one on publish). Show it as the fallback image while the viewer loads or if WebGPU is unavailable.

10. **Loading state** — Show a spinner or progress bar while `loadUrl()` is in flight (especially for large `.frogmarks` files with GLBs).

11. **CDN publish flow** — In Frogmarks, add a "Publish" button that calls `shapeManager.snapshotDocument()` → `packProject()` → uploads the resulting `Blob` to object storage (S3/R2/GCS) → returns a public URL. The user can then copy `<salsa-viewer url="...">` from a share dialog.
