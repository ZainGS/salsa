...
| **Bleed / diffusion** | ✅ Shipped | Medium | High |
| **Color mixing (smudge)** | ✅ Shipped | High | High |
...
| Method | Signature | Description |
|--------|-----------|-------------|
| `setBrushBleed(id, settings)` | `(id: string, settings: BleedSettings) → boolean` | Configure paint bleed/diffusion. Set `perDab: true` for live spread during stroke, or `false` for a single end-of-stroke blur pass. |

### Color Mixing / Smudge (NEW)

| Method | Signature | Description |
|--------|-----------|-------------|
| `setBrushSmudge(id, settings)` | `(id: string, settings: SmudgeSettings) → boolean` | Configure smudge (finger-smear). Uses async GPU readback with 1-dab lag to mix canvas color into stroke color. |
