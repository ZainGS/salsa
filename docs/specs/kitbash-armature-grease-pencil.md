...
**Status:**
- **Phase A — Armatures: ✅ COMPLETE** (implemented May 2026 — see `docs/reference/19-armature-system.md`)
- **Phase B — Kitbashing: ✅ COMPLETE** (implemented 2026-05-10)
- **Phase C — Grease Pencil: ✅ COMPLETE** (implemented 2026-05-10)
...
### Phase A — Armature Engine ✅ COMPLETE (May 2026)

All items implemented. See `docs/reference/19-armature-system.md` for full engine documentation.

| Step | What | Status |
|------|------|--------|
| A1 | `src/types/armature-3d.ts` — all types | ✅ Done |
| A2 | `Skeleton3D` scene node with `computeWorldMatrices()` | ✅ Done |
| A3 | `SkinnedMesh3D` scene node — holds joint/weight buffers | ✅ Done |
| A4 | 72-byte skinned vertex format, GPU upload in `Renderer3D` | ✅ Done |
| A5 | Skinning WGSL vertex shaders (textured + untextured variants) | ✅ Done |
| A6 | Skinned pipelines in `Pipeline3D` (`_skinBGL`, group layout shifting) | ✅ Done |
| A7 | `Renderer3D.drawSkinnedMeshes()` — per-mesh VB/IB/skinMatBuf, separate inst buffer | ✅ Done |
| A8 | GLTF importer extension — `parseSkinnedGLB/GLTF`, reads skins/JOINTS_0/WEIGHTS_0 | ✅ Done |
| A9 | `applySkeletonClipAtFrame()` + `playSkeletonClip3D()` (quat.slerp + linear interp) | ✅ Done |
| A10 | Bone gizmo drawing in `GizmoRenderer` — diamond bone sticks + joint spheres | ✅ Done |
| A11 | Joint picking — `hitTestJoint()` + hover/select via mouse events in Scene3DManager | ✅ Done |
| A12 | ShapeManager API: `importSkinnedGltfFile3D`, `setJointRotation3D`, `playSkeletonClip3D`, `getSelectedJoint3D`, `selectJoint3D` | ✅ Done |
| A13 | Serialization: `{ nodes, skeletons }` format, `Skeleton3D.fromJSON`, base64 joint data, full save/restore flow | ✅ Done |

---

### Phase B — Kitbashing Library ✅ COMPLETE (2026-05-10)

#### Shipped implementation

| Step | What | Status | Key files |
|------|------|--------|-----------|
| B1 | `src/types/kitbash-3d.ts` — `CharacterSlot`, `KitbashPartMeta`, `CharacterDefinition`, `CharacterData` | ✅ | New file |
| B2 | `KitbashLibrary` — manifest fetch, `addParts()`, `getPartsBySlot()`, `getPart()` | ✅ | `src/services/managers/kitbash-library.ts` |
| B3 | Character assembly in `Scene3DManager` — `createCharacter()`, `swapCharacterSlot()`, `setCharacterSlotColor()`, `removeCharacter()` | ✅ | `src/services/managers/scene3d-manager.ts` |
| B4 | Joint remapping — `_remapJointIndices()` maps part-local `JOINTS_0` to canonical skeleton indices by joint name | ✅ | `scene3d-manager.ts` |
| B5 | ShapeManager API: `loadKitbashLibrary3D`, `addKitbashParts3D`, `getKitbashParts3D`, `createCharacter3D`, `swapCharacterSlot3D`, `setCharacterSlotColor3D`, `removeCharacter3D`, `getCharacterDefinition3D`, `getAllCharacters3D` | ✅ | `shape-manager.ts` |
| B6 | Serialization: `getScene3DCharacterStates()` + `restoreCharacterStates()` + `characters3d` in `scene3d.json` | ✅ | `scene3d-manager.ts`, `project-package.ts`, `shape-manager.ts` |
| B7 | **Initial asset pack** — 1 base body + 3 hair + 3 tops + 2 bottoms + 2 shoes | 📋 Art production | n/a |
| B8 | Frogmarks character creator UI | 📋 Frogmarks (Angular) | see UI spec below |

#### Architecture note

`CharacterData` is stored in `Scene3DManager._characterMap` (not a scene-graph node). The `Skeleton3D` and `SkinnedMesh3D` nodes are added to the scene graph root directly — the same way standalone GLTF imports work — and are picked up naturally by `getAllMeshes()`/`getAllSkeletons()`. `CharacterData` is the organizational record that links them under a named character with slot assignments.

**Validation test:** Load manifest, call `sm.createCharacter3D({ id, name, slots: { base_body: 'body-001', hair: 'hair-wavy-001' } })`, verify `Skeleton3D` + two `SkinnedMesh3D` appear in scene, call `sm.swapCharacterSlot3D(charId, 'hair', 'hair-bob-002')`, verify mesh is replaced.

---

### Phase C — Grease Pencil ✅ COMPLETE (2026-05-10)

| Step | What | Status | Key files |
|------|------|--------|-----------|
| C1 | `src/types/grease-pencil-3d.ts` — types | ✅ | New file |
| C2 | `GpObject3D` scene node | ✅ | `src/scene-graph/shapes/gp-object-3d.ts` |
| C3 | `GpRenderer3D` — CPU point buffer, quad-strip expand shader | ✅ | `src/renderer/3d/gp-renderer-3d.ts` |
| C4 | GP stroke shader + fill shader | ✅ | `src/renderer/3d/shaders/gp-shaders.ts` |
| C5 | Fill triangulation (standalone earClip, Newell's method projection) | ✅ | `gp-renderer-3d.ts` |
| C6 | Bone parenting in shader (per-skeleton GPU skinMatrices buffer) | ✅ | `gp-shaders.ts`, `gp-renderer-3d.ts` |
| C7 | ShapeManager API: `createGpObject3D`, `beginGpStroke3D`, `addGpPoint3D`, `endGpStroke3D`, `eraseGpStrokes3D`, `addGpLayer3D`, `removeGpLayer3D`, `removeGpObject3D` | ✅ | `shape-manager.ts`, `scene3d-manager.ts` |
| C8 | Surface snap: ray-cast to character mesh for stroke placement depth | 📋 | `mesh-picker.ts` extension (not yet needed) |
| C9 | Keyframe animation: `setGpKeyframe3D`, `clearGpKeyframe3D` | ✅ | `shape-manager.ts`, `GpObject3D` |
| C10 | Serialization: `GpObject3D.toJSON()` + project-package `gpObjects` field | ✅ | `project-package.ts`, `shape-manager.ts` |
| C11 | Renderer wired into WebGPU render loop after particles | ✅ | `webgpu-renderer.ts` |
| C12 | Frogmarks GP drawing tool UI | 📋 | Frogmarks (Angular) — see `docs/ui/grease-pencil.md` |
...
## GP Layer Z-Sort / Render Order

**Status: ✅ Shipped — 2026-05-10**

Each `GpObject3D` has a `renderOrder: number` field (default 0). `draw3DGp()` sorts GP objects by `renderOrder` (ascending) before drawing. Higher values draw on top within the GP group.

Public API: `sm.setGpRenderOrder3D(gpId, order)`.

Serialized in `GpObject3D.toJSON()` / `fromJSON()` — persists across save/load.

### Full Z-sort (future)

To allow a GP object to render **between** meshes and particles, the render loop must be refactored to dispatch by a unified scene-level `renderOrder` across all object types. Design this when a real scene demands it.
