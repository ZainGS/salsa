...
- **Self-describing worldParams MARKER ✅ (the primary, robust re-adoption mechanism — matches Buildings/Foliage).** Every package root also carries a **`worldParams = { kind:'packaging', entry }`** marker, where `entry` is the full params-only persist record (style, params, fold, dieline layer, layer-stack order + active, board). It is **(re)stamped on every state change** (`create`/`addPackage`, `setDimensions`, `setStyle`, `setFoldAmount`, board/dieline/layer-stack edits, and after re-adoption) via `PackagingManager._stampMarker`. Because the root is a `documentSkipChildren` thin-wrapper, `worldParams` **round-trips through sceneGraphJSON** (the panels are skipped, regenerated from `entry.params` on load) — so a package is **self-describing** and re-adoptable from the scene graph ALONE, needing **no separate `packaging` array**. On load, `PackagingManager.restoreFromSave()` scans the scene root for these markers (host hook `findPackageMarkers()` → `getRootMeshGroups()` filtered by `worldParams.kind`, exactly like `building-manager.restoreFromSave`) and re-adopts each from its `entry` through the same regenerate-under-existing-root path. It runs **eagerly** from `restoreProceduralFromSave3D()` alongside buildings/foliage, so `getAll()`/`isPackageNode()` — the 📦 icon, delete, and Package-Mode-on-select — work the **instant a document loads**, not only after the user enters creator mode. **Idempotent** (already-registered ids are skipped), so it is safe next to the legacy `restoreFromJSON(parsed.packaging)` array path in `restoreDocumentState` (whichever runs first wins; the other adopts nothing — no duplicate box). *Root cause of the old empty-`getAll` bug: re-adoption depended solely on the separate scene3dJSON `packaging` array; if it was empty/missing (or a prior failed reload cascaded), the marker roots still restored (visible in the Outliner) but the registry stayed empty. The self-describing marker removes that single point of failure.* Legacy markers carrying only `kind` (no `entry`) fall back gracefully to the structural orphan sweep — never crash.
