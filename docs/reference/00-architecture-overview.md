...
| 15 | [3D Rendering System](15-3d-rendering-system.md) | Renderer3D, Pipeline3D, Camera3D, OrbitController, GizmoRenderer, MeshPicker (barycentric PickResult), FrustumCuller, shadow mapping, PS1 aesthetics, TextureLibrary, skinned mesh pipeline, bone overlay, MeshPaintManager |
| 16 | [3D Animation System](16-3d-animation-system.md) | Keyframe tracks, interpolation, AnimationPlayer3D, UndoManager3D, raster↔3D playback sync, skeleton animation, joint picking |
...
| 19 | [Armature & Skeletal Animation System](19-armature-system.md) | Skeleton3D, SkinnedMesh3D, LBS skinning, GLTF skin import, skeleton animation clips, bone gizmo, joint picking |
...
└──────┬──────────┬──────────┬──────────┬─────────────────────┘
  ...
  SceneGraph  InteractionSvc  Services  WebGPURenderer
    ...
    (Shapes    Pan/Zoom    Services   ┌──────────┐
     ...
     +Skeleton3D
     +ClothMesh3D
     +ParticleEmitter3D)                         │beginFrame│──▶ Cache System
                                    ...
                                    GPU Render Pass (single pass)
                                     ...
                                     │   ├─ Skinned meshes (LBS vertex shader, per-mesh VB)
                                     │   ├─ Transparent meshes
                                     │   ├─ Particles (billboard quad strips)
                                     │   └─ Gizmos (move/rotate/scale + bone overlay)
...
### 3D Mesh Rendering
A fully self-contained 3D rendering system (`Renderer3D`) injects into the shared render pass between the background and foreground raster layers. Features: Gouraud lighting with Phong specular, PS1-style vertex snapping and color quantization, PCF soft shadow mapping, frustum culling, interactive transform gizmos, keyframe animation, a shared texture library for multi-mesh texture reuse, GPU particle emitters, and skeletal animation with Linear Blend Skinning (LBS).
