...
export function buildSkyway(graph: WorldGraph): LayoutPreviewLayer[] {
    ...
    return [{ name: 'world:rail-sky', color: [0.30, 0.90, 1.0], y: gy, geometry: beam.geometry(), emissive: 1.2, opacity: 0.8 }];
}
...
export function buildRailway(graph: WorldGraph, includeTrain = true): LayoutPreviewLayer[] {
    ...
    // Piers (skip canals — the deck spans them). Base buried, top stops just under the deck slab.
    const nP = Math.max(3, Math.round((z1 - z0) / (1.1 * s)));
    const pierBase = gy - 0.3 * s, pierTop = deckY - 0.015 * s;
    for (let i = 0; i <= nP; i++) {
        const z = z0 + (z1 - z0) * i / nP;
        if (cellLevelAt(graph, rx, z) < 0) continue;
        pier.prism([rx, pierBase, z], 0.03 * s, 0.03 * s, pierTop - pierBase, 6);
        pier.obox([rx, pierTop - 0.012 * s, z], zA, up, xA, 0.006 * s, 0.012 * s, w * 1.4);   // pier cap (top flush at pierTop, under the slab)
    }
    ...
    if (!pier.empty) out.push({ name: 'world:rail-pier', color: PIER, y: gy, geometry: pier.geometry() });
    if (!deck.empty) out.push({ name: 'world:rail-deck', color: DECK, y: gy, geometry: deck.geometry() });
    if (!rail.empty) out.push({ name: 'world:rail-track', color: RAIL, y: gy, geometry: rail.geometry() });
    if (!train.empty) out.push({ name: 'world:rail-train', color: TRAIN, y: gy, geometry: train.geometry() });
    if (!trainDk.empty) out.push({ name: 'world:rail-train-win', color: TRAIN_DK, y: gy, geometry: trainDk.geometry(), emissive: p.nightMode ? 0.95 : 0.5 });
    return out;
}
